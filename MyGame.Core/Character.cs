...
namespace MyGame.Core
{
    public abstract class Character
    {
        ...
        public abstract string Attack(Character target); //metoda ataku, która jest abstrakcyjna, ponieważ każda postać atakuje inaczej
        ...
        {
            string message;
            if (Health == MaxHealth) message="Cannot heal at full health.";
            Health = (int)(Health + (MaxHealth - Health) * 0.7);
            message=$"{Name} uses a healing item and restores 70% of their missing health!";
            HealingItem--;
            return message;
        }
        ...
        public abstract string UseAbility(Character target);
